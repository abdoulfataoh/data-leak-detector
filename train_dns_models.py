...
dataset = load_dataset(settings.GOOD_DNS_DATASET, settings.MALICIOUS_DNS_DATASET)

dataset['query_length'] = dataset['query'].str.len()
features_labels = [
            'AA', 'RA', 'RD', 'TC', 'Z',
...
# save to_mtrix model
with open(settings.TO_MATRIX_DNS, 'wb') as file:
  pickle.dump(to_matrix, file)

# save to_mtrix model
with open(settings.RANDOM_FOREST_DNS_MODEL, 'wb') as file:
